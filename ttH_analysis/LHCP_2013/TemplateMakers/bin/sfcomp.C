{
  TFile * f0 = new TFile("csv_rwt_hf_new.root");
  TFile * f1 = new TFile("csv_rwt_hf_IT_v1.root");
  TFile * f2 = new TFile("csv_rwt_hf_IT_v2.root");

//   int nPt = 3;
//   int nEta = 3;
//   double xBins[17] = {-0.08, -0.04, 0.0, 0.04, 0.08, 0.12, 0.16, 0.2, 0.244, 0.331, 0.418, 0.505, 0.592, 0.679, 0.752, 0.825, 1.01};
//   int nBins = 16;
  // heavy flavor Bins
  int nPt = 5;
  int nEta = 1;
  double xBins[19] = {-0.1, 0.0, 0.122, 0.244, 0.331, 0.418, 0.505, 0.592, 0.679, 0.7228, 0.7666, 0.8104, 0.8542, 0.898, 0.9184, 0.9388, 0.9592, 0.9796, 1.01};
  int nBins = 18;
  ...
  for (unsigned int iPt = 0; iPt < nPt; iPt++){
    for (unsigned int iEta = 0; iEta < nEta; iEta++){
      ...
      TString canvName = Form("hf_Pt%i_Eta%i_compare",iPt,iEta);  
	...
	h2->SetBinError(iBin, h2temp->GetBinError(iBin));
      ...
      h1->SetMarkerColor(2);
      h2->SetMarkerColor(3);
      if ( h0->GetMaximum() < h1->GetMaximum()) h0->SetMaximum( 1.1*(h1->GetMaximum())); ///
      canv->cd();
      gStyle->SetOptStat(0);
      h0->Draw("pe");
      h1->Draw("pesame");
