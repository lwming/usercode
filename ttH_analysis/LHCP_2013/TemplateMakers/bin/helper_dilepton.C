...
//     floatBranches["leps_minus_tagged_jets_charge"] = new float(0.0);
    ...
    floatBranches["first_jet_JP"] = new float(0.0);
    floatBranches["second_jet_JP"] = new float(0.0);
    floatBranches["first_jet_TCHP"] = new float(0.0);
    floatBranches["second_jet_TCHP"] = new float(0.0);
    floatBranches["third_jet_CSV_unc"] = new float(0.0);
    floatBranches["fourth_jet_CSV_unc"] = new float(0.0);

    floatBranches["first_jet_SSV"] = new float(0.0);
    floatBranches["second_jet_SSV"] = new float(0.0);
    floatBranches["first_jet_SSVHE"] = new float(0.0);
    floatBranches["second_jet_SSVHE"] = new float(0.0);
    floatBranches["first_jet_SSVHP"] = new float(0.0);
    floatBranches["second_jet_SSVHP"] = new float(0.0);
...
//     intBranches["PassZmask3"] = new int (0);
    ...
    floatBranches["avg_dEta_jets"] = new float(0.0);    
    floatBranches["max_dEta_jets"] = new float(0.0);    
    ...
    for( ev.toBegin(); !ev.atEnd(); ++ev) {
     ...
     //// specific to ttH signal sample???
      ...
      if( (sampleName == "ttbar" || tmpName.Contains("ttbar_" )) && !tmpName.Contains("ttbar_scale") && !tmpName.Contains("ttbar_matching")) {       
        ...
        keepEvent = beanHelper.ttPlusHFKeepEvent (mcparticles, pfjets );
//         keepEvent = beanHelper.ttPlusHeavyKeepEvent (mcparticles, pfjets );
      ...
      //-------------------------------------
      //------------  split ttH by decay mode -------//
      //-------------------------------------
      if ( tmpName.Contains("ttH") ) {
        bool keepttHEvent = false;

	hdecayType::hdecayType decayTypeTmp ;
	if (tmpName.Contains("hgg")) decayTypeTmp = hdecayType::hgg;
	else if (tmpName.Contains("hbb")) decayTypeTmp = hdecayType::hbb;
	else if (tmpName.Contains("hcc")) decayTypeTmp = hdecayType::hcc;
	else if (tmpName.Contains("hww")) decayTypeTmp = hdecayType::hww;
	else if (tmpName.Contains("hzz")) decayTypeTmp = hdecayType::hzz;
	else if (tmpName.Contains("htt")) decayTypeTmp = hdecayType::htt;
	else if (tmpName.Contains("hjj")) decayTypeTmp = hdecayType::hjj;
	else if (tmpName.Contains("hzg")) decayTypeTmp = hdecayType::hzg;
	else {
	  std::cout << "Error, undefined decay type!" << std::endl;
	  break;
	}

        keepttHEvent = beanHelper.keepHdecayType(mcparticles, decayTypeTmp );

        if(!keepttHEvent) continue;	     
      }
...
//       break;
	...
	jet_CSV_sorted.push_back(*listint);  //accessible form
      ...
      float first_jet_JP = dFloat ;
      float second_jet_JP = dFloat ;
      float first_jet_TCHP = dFloat ;
      float second_jet_TCHP = dFloat ;
      float first_jet_SSV = dFloat ;
      float second_jet_SSV = dFloat ;
      float first_jet_SSVHE = dFloat ;
      float second_jet_SSVHE = dFloat ;
      float first_jet_SSVHP = dFloat ;
      float second_jet_SSVHP = dFloat ;
      ...
      float avg_dEta_jets = dFloat;
      float max_dEta_jets = dFloat;
    ...
    BNmcparticle MCLep2;
	  ...
	  for (int i=0; i < numGoodJets; i++) {
	    ...
	    if (i==0)  {        
          ...
          first_jet_JP = pfjetsSelected.at(iJet).btagJetProb;
          first_jet_TCHP = pfjetsSelected.at(iJet).btagTChighPur;
          first_jet_SSV = pfjetsSelected.at(iJet).btagSecVertex;
          first_jet_SSVHE = pfjetsSelected.at(iJet).btagSecVertexHighEff;
          first_jet_SSVHP = pfjetsSelected.at(iJet).btagSecVertexHighPur;
	  ...
	  //	  if(first_jet_genID!=first_jet_flavor) cout << "genID and flavor dont match: genID:flavor "<< first_jet_genID << ":" << first_jet_flavor  << endl;
	    ...
	    if (i==1)  {
          ...
          second_jet_JP = pfjetsSelected.at(iJet).btagJetProb;
          second_jet_TCHP = pfjetsSelected.at(iJet).btagTChighPur;
          second_jet_SSV = pfjetsSelected.at(iJet).btagSecVertex;
          second_jet_SSVHE = pfjetsSelected.at(iJet).btagSecVertexHighEff;
          second_jet_SSVHP = pfjetsSelected.at(iJet).btagSecVertexHighPur;
	  ...
	  if ((numGoodJets - numTaggedJets) != 0){
	    if (numNonbjetsDom == 0) avg_btag_disc_non_btags = 0; //// was -1;  ///// what value
...
//       }
	  ...
	  if(numJets > 1 ) {
        ...
        if (avg_dEta_jets == dFloat) avg_dEta_jets = 0.0;
        if (max_dEta_jets == dFloat) max_dEta_jets = 0.0;
		avg_dr_jets += jet_vect1.DeltaR(jet_vect2);
		avg_dEta_jets += fabs(jet_vect1.Eta() - jet_vect2.Eta());
		if ( fabs(jet_vect1.Eta() - jet_vect2.Eta()) > max_dEta_jets) max_dEta_jets = fabs(jet_vect1.Eta() - jet_vect2.Eta());
		++denom_avg_cnt;		      
...
//         *(floatBranches["leps_minus_tagged_jets_charge"]) = fabs(lep1TkCharge+lep2TkCharge-sum_tagged_jet_charge);
        ...
        *(floatBranches["first_jet_JP"]) = first_jet_JP;
        *(floatBranches["second_jet_JP"]) = second_jet_JP;
        *(floatBranches["first_jet_TCHP"]) = first_jet_TCHP;
        *(floatBranches["second_jet_TCHP"]) = second_jet_TCHP;
        *(floatBranches["third_jet_CSV_unc"]) = third_jet_CSV_unc;
        *(floatBranches["fourth_jet_CSV_unc"]) = fourth_jet_CSV_unc;

        *(floatBranches["first_jet_SSV"]) = first_jet_SSV;
        *(floatBranches["second_jet_SSV"]) = second_jet_SSV;
        *(floatBranches["first_jet_SSVHE"]) = first_jet_SSVHE;
        *(floatBranches["second_jet_SSVHE"]) = second_jet_SSVHE;
        *(floatBranches["first_jet_SSVHP"]) = first_jet_SSVHP;
        *(floatBranches["second_jet_SSVHP"]) = second_jet_SSVHP;
...
//         *(intBranches["PassZmask3"]) = PassBigDiamondZmask3 ? 1 : 0;
	...
	*(floatBranches["avg_dEta_jets"]) = avg_dEta_jets;
	*(floatBranches["max_dEta_jets"]) = max_dEta_jets;
