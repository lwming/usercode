...
//   Float_t                 varsum_jet_pt;
  ...
  for( unsigned int j = 0 ; j < nCat ; ++j ){
	...
	reader[j]->AddVariable( "higgsLike_dijet_mass", &varhiggsLike_dijet_mass );
	reader[j]->AddVariable( "higgsLike_dijet_mass2", &varhiggsLike_dijet_mass2 );
	if (j==8 || j==11)  reader[j]->AddVariable( "numHiggsLike_dijet_15_float", &varnumHiggsLike_dijet_15_float );
	if (j==7 || j==10)  reader[j]->AddVariable( "numJets_float", &varnumJets_float );
	reader[j]->AddVariable( "sum_pt", &varsum_pt );
	if (j!=7 && j!=10)  reader[j]->AddVariable( "avg_btag_disc_btags",   &varavg_btag_disc_btags   ); 
	if (j!=8 && j!=11)  reader[j]->AddVariable( "avg_btag_disc_non_btags",   &varavg_btag_disc_non_btags   ); 
      }
//       else if (j==6){ //e3je2t
// 	reader[j]->AddVariable( "first_jet_pt",         &varfirst_jet_pt   );
// 	reader[j]->AddVariable( "avg_dr_jets",         &varavg_dr_jets   ); ///
...
//       }
      ...
      if (8<j && j<12) wfName = dir + prefix + TString("_BDTG.weights.xml");

      std::cout << "Loading  weight file  " << wfName << std::endl; 
      
      // Book method(s)
      if (8<j && j<12) reader[j]->BookMVA( "BDTG method", wfName);
