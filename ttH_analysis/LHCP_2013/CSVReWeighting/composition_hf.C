...
#include <algorithm>
  ...
  gStyle->SetOptStat(0);
  TString ofName = "checkComposition/composition_hf.root";
...
//   TString label1 = "limits_v1"; //Label of the input trees
//   TString label2 = "limits_v1"; //Label of the input trees
//   TString extraLabel = ""; //Systematic of the input trees; set to "JESUp_" for JESUp, etc.
  TString WP = "-100"; //100x probe jet working point - loose (224), medium (679), tight (898)
...
//   etaBins.push_back("1.6");
  ...
  TFile * sourceFile1a = new TFile ("/afs/crc.nd.edu/user/w/wluo1/LHCP_2013/CMSSW_5_3_8_patch1/src/BEAN/csvReWeighting_DIR/TwoMuon.root");
  TFile * sourceFile1b = new TFile ("/afs/crc.nd.edu/user/w/wluo1/LHCP_2013/CMSSW_5_3_8_patch1/src/BEAN/csvReWeighting_DIR/TwoEle.root");
  TFile * sourceFile1c = new TFile ("/afs/crc.nd.edu/user/w/wluo1/LHCP_2013/CMSSW_5_3_8_patch1/src/BEAN/csvReWeighting_DIR/MuonEle.root");
  TFile * sourceFile2 = new TFile  ("/afs/crc.nd.edu/user/w/wluo1/LHCP_2013/CMSSW_5_3_8_patch1/src/BEAN/csvReWeighting_DIR/allMC.root");
...
//   TString jetselection1b = "(numJets == 2 && first_jet_CSV_unc < 0.244)"; //Tag jet requirement for probing second_jet
  ...
  TString firstjetb = "( abs(first_jet_flavor)==5 )"; 
  TString secondjetb = "( abs(second_jet_flavor)==5 )";
  TString sampleCut = "(sampleNumber==2533 || sampleNumber==2563 || sampleNumber==2566)"; 
...
//   //  Double_t xBins[13] = {30,40,50,60,70,80,100,120,160,210,260,320,800};
  ...
  int nBins = 17; //Number of bins
//   Double_t xBins[16] = {0,0.061,0.122,0.183,0.244,0.389,0.534,0.679,0.752,0.825,0.898,0.932,0.966,0.99,0.999,1.0}; //Binning
  double xBins[18] = {0.0, 0.122, 0.244, 0.331, 0.418, 0.505, 0.592, 0.679, 0.7228, 0.7666, 0.8104, 0.8542, 0.898, 0.9184, 0.9388, 0.9592, 0.9796, 1.01}; //// no underflow
...
//   TH1 * sourceHisto2a = (TH1*) summaryTree->Draw(varName2+" >> histTemp2a",nonttbar+lumi+"weight*(Xsec/nGen)*("+lepselection2+ " && "+trigselection+" && "+jetselection1b+")","");
  TH1 * sourceHisto3a = (TH1*) summaryTree->Draw(varName1+" >> histTemp3a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && "+firstjetb+" && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto4a = (TH1*) summaryTree->Draw(varName2+" >> histTemp4a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && "+secondjetb+" && "+trigselection+" && "+jetselection2b+")","");
  TH1 * sourceHisto5a = (TH1*) summaryTree->Draw(varName1+" >> histTemp5a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && !"+firstjetb+" && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto6a = (TH1*) summaryTree->Draw(varName2+" >> histTemp6a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && !"+secondjetb+" && "+trigselection+" && "+jetselection2b+")","");
...
//   histTemp5a->Scale(Data_tagged/MC_tagged);
  ...
  /////////////
  TH1F *histTemp3b = new TH1F("histTemp3b", "histTemp3b", nBins, xBins);
  TH1F *histTemp4b = new TH1F("histTemp4b", "histTemp4b", nBins, xBins);
  TH1F *histTemp5b = new TH1F("histTemp5b", "histTemp5b", nBins, xBins);
  TH1F *histTemp6b = new TH1F("histTemp6b", "histTemp6b", nBins, xBins);
  histTemp3b->Sumw2();
  histTemp4b->Sumw2();
  histTemp5b->Sumw2();
  histTemp6b->Sumw2();
  TH1 * sourceHisto3b = (TH1*) summaryTree->Draw(varName1+" >> histTemp3b",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+ sampleCut + " && "+lepselection2+ " && "+firstjetb+" && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto4b = (TH1*) summaryTree->Draw(varName2+" >> histTemp4b",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+ sampleCut + " && "+lepselection2+ " && "+secondjetb+" && "+trigselection+" && "+jetselection2b+")","");
  TH1 * sourceHisto5b = (TH1*) summaryTree->Draw(varName1+" >> histTemp5b",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+ sampleCut + " && "+lepselection2+ " && !"+firstjetb+" && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto6b = (TH1*) summaryTree->Draw(varName2+" >> histTemp6b",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+ sampleCut + " && "+lepselection2+ " && !"+secondjetb+" && "+trigselection+" && "+jetselection2b+")","");

  histTemp3b->Add(histTemp4b);
  histTemp5b->Add(histTemp6b);

  histTemp3b->SetMarkerStyle(8);
  histTemp3b->SetName(Form("csv_MC_bjets_ttbar_Pt%i_Eta%i",iPt,iEta));

  histTemp5b->SetMarkerStyle(8);
  histTemp5b->SetName(Form("csv_MC_nonbjets_ttbar_Pt%i_Eta%i",iPt,iEta));

  /////////////
  TH1F *hClonehf = (TH1F*)histTemp3b->Clone(Form("csvhf_ttbarRatio_Pt%i_Eta%i",iPt,iEta));
  TH1F *hClonelf = (TH1F*)histTemp5b->Clone(Form("csvlf_ttbarRatio_Pt%i_Eta%i",iPt,iEta));
  hClonehf->Divide(hClonehf,histTemp3a);
  hClonelf->Divide(hClonelf,histTemp5a);

  histTemp3a->SetDirectory(outputFile);
  histTemp5a->SetDirectory(outputFile);
  histTemp3b->SetDirectory(outputFile);
  histTemp5b->SetDirectory(outputFile);
  hClonehf->SetDirectory(outputFile);
  hClonelf->SetDirectory(outputFile);
...
//   }
  ...
  else hClonehf->Draw("histE1");

  TString plotName = Form("_Pt%i_Eta%i",iPt,iEta);
  can1a->Print("checkComposition/"+varName+plotName +".png");

//   can1a->Print("BtagReshaping/Efficiency_"+varName+"_ptCut_"+pt_lower+"_to_"+pt_upper+"_"+extraLabel+lfSF+".png");
