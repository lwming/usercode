...
#include <algorithm>
  ...
  TString ofName = "abc_csv_rwt_hf.root";
...
//   TFile * sourceFile2 = new TFile ("bin/dilSummaryTrees_"+MCsample1+"_"+year+"_"+label2+"_"+extraLabel+"all.root");
  ...
  //double xBins[21] = {0.0, 0.0488, 0.0976, 0.1464, 0.1952, 0.244, 0.331, 0.418, 0.505, 0.592, 0.679, 0.7228, 0.7666, 0.8104, 0.8542, 0.898, 0.9184, 0.9388, 0.9592, 0.9796, 1};
  double xBins[17] = {0.0, 0.244, 0.331, 0.418, 0.505, 0.592, 0.679, 0.7228, 0.7666, 0.8104, 0.8542, 0.898, 0.9184, 0.9388, 0.9592, 0.9796, 1};
  }
...
//   TH1 * sourceHisto2a = (TH1*) summaryTree->Draw(varName2+" >> histTemp2a",nonttbar+lumi+"weight*(Xsec/nGen)*("+lepselection2+ " && "+trigselection+" && "+jetselection1b+")","");
  TH1 * sourceHisto3a = (TH1*) Tree2->Draw(varName1+" >> histTemp3a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && "+firstjetb + " && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto4a = (TH1*) Tree2->Draw(varName2+" >> histTemp4a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && "+secondjetb +" && "+trigselection+" && "+jetselection2b+")","");
  TH1 * sourceHisto5a = (TH1*) Tree2->Draw(varName1+" >> histTemp5a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && !"+firstjetb +" && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto6a = (TH1*) Tree2->Draw(varName2+" >> histTemp6a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && !"+secondjetb +" && "+trigselection+" && "+jetselection2b+")","");
...
//   hClone->Write();
  ...
  can1a->Print("test/"+varName+"_"+plotName +".png");
