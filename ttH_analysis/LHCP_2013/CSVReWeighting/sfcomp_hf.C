{
  ...
  TFile * f1 = new TFile("csv_rwt_hf_IT_v2_JESUp.root");
  TFile * f2 = new TFile("csv_rwt_hf_IT_v2_JESDown.root");

//   int nPt = 3;
//   int nEta = 3;
//   double xBins[16] = {-0.05, 0.0, 0.04, 0.08, 0.12, 0.16, 0.2, 0.244, 0.331, 0.418, 0.505, 0.592, 0.679, 0.752, 0.825, 1.01};
//   int nBins = 15;
  // heavy flavor Bins
  int nPt = 5;
  int nEta = 1;
  ...
  for (unsigned int iPt = 0; iPt < nPt; iPt++){
    for (unsigned int iEta = 0; iEta < nEta; iEta++){
      ...
      TString canvName = Form("hf_Pt%i_Eta%i_JEScompare",iPt,iEta);  
