...
#include <algorithm>
  ...
  gStyle->SetOptStat(0);
  TString ofName = "composition_lf.root";
...
//   TString jetselection1b = "(numJets == 2 && first_jet_CSV_unc < 0.244)"; //Tag jet requirement for probing second_jet
  TString jetselection2a = "(numJets == 2 && second_jet_CSV_unc < 0.244 "; //Probe jet requirement for first_jet
  if (WP != "-100") jetselection2a += " && first_jet_CSV_unc > 0."+WP;
  TString jetselection2b = "(numJets == 2 && first_jet_CSV_unc < 0.244 "; //Probe jet requirement for second_jet
...
//  double xBins[18] = {0.0, 0.0488, 0.0976, 0.1464, 0.1952, 0.244, 0.331, 0.418, 0.505, 0.592, 0.679, 0.7228, 0.7666, 0.8104, 0.8542, 0.898, 0.949, 1};
  double xBins[17] = {0.0, 0.04, 0.08, 0.12, 0.16, 0.2, 0.244, 0.331, 0.418, 0.505, 0.592, 0.679, 0.752, 0.825, 0.898, 0.949, 1.01};
...
//   histTemp5a->Scale(Data_tagged/MC_tagged);
  ...
  histTemp5b->Sumw2();
  histTemp6b->Sumw2();
  TH1 * sourceHisto3b = (TH1*) summaryTree->Draw(varName1+" >> histTemp3b",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+ zjetsSample + " && "+lepselection2+ " && "+firstjetb+" && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto4b = (TH1*) summaryTree->Draw(varName2+" >> histTemp4b",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+ zjetsSample + " && "+lepselection2+ " && "+secondjetb+" && "+trigselection+" && "+jetselection2b+")","");
  TH1 * sourceHisto5b = (TH1*) summaryTree->Draw(varName1+" >> histTemp5b",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+ zjetsSample + " && "+lepselection2+ " && !"+firstjetb+" && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto6b = (TH1*) summaryTree->Draw(varName2+" >> histTemp6b",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+ zjetsSample + " && "+lepselection2+ " && !"+secondjetb+" && "+trigselection+" && "+jetselection2b+")","");

  histTemp3b->Add(histTemp4b);
  histTemp5b->Add(histTemp6b);

  histTemp3b->SetMarkerStyle(8);
  histTemp3b->SetName(Form("csv_MC_bjets_zjets_Pt%i_Eta%i",iPt,iEta));

  histTemp5b->SetMarkerStyle(8);
  histTemp5b->SetName(Form("csv_MC_nonbjets_zjets_Pt%i_Eta%i",iPt,iEta));

  /////////////
  TH1F *hClonehf = (TH1F*)histTemp3b->Clone(Form("csvhf_zjetsRatio_Pt%i_Eta%i",iPt,iEta));
  TH1F *hClonelf = (TH1F*)histTemp5b->Clone(Form("csvlf_zjetsRatio_Pt%i_Eta%i",iPt,iEta));
