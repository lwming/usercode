...
#include <algorithm>
  ...
  gStyle->SetOptStat(0);
//   TString ofName = "pileUp_test.root";
...
//   TFile * sourceFile1c = new TFile ("../csvReWeighting_DIR/MuonEle.root");
  TFile * sourceFile2 = new TFile  ("../csvReWeighting_DIR/rootFile_v0/allMC.root");
...
//   TString jetselection1b = "(numJets == 2 && first_jet_CSV_unc < 0.244)"; //Tag jet requirement for probing second_jet
  ...
  TString jetselection = "(numJets == 2 && numTaggedJets >= 2)";   /////
...
//     TString varName2 = "second_jet_eta"; //Variable which the scale factor is a function of
  ...
  TString varName =  "numPV"; //Variable which the scale factor is a function of
  TString varName1 = "numPV"; //Variable which the scale factor is a function of
  TString varName2 = "numPV"; //Variable which the scale factor is a function of
  }
...
//   //  Double_t xBins[13] = {30,40,50,60,70,80,100,120,160,210,260,320,800};
  ...
  double Xmax = 51; //Upper bound for x-axis
  int nBins = 51; //Number of bins
...
//   histTemp5a->Sumw2();
//   histTemp6a->Sumw2();
  TH1 * sourceHisto3a = (TH1*) summaryTree->Draw(varName1+" >> histTemp3a",nonttbar+lumi+"weight*topPtWgt*csvWgtlf*csvWgthf*(Xsec/nGen)*("+lepselection2+ " && "+trigselection+" && "+jetselection+")","");
//   TH1 * sourceHisto4a = (TH1*) summaryTree->Draw(varName2+" >> histTemp4a",nonttbar+lumi+"weight*topPtWgt*csvWgthf*(Xsec/nGen)*("+lepselection2+ " && "+trigselection+" && "+jetselection+ " && !" +zjetsSample +")","");
//   TH1 * sourceHisto5a = (TH1*) summaryTree->Draw(varName1+" >> histTemp5a",nonttbar+lumi+"weight*topPtWgt*csvWgthf*(Xsec/nGen)*("+lepselection2+ " && !"+firstjetb+" && "+trigselection+" && "+jetselection+")","");
...
//   histTemp3a->Scale(Data_tagged/MC_tagged);
  ...
  histTemp3a->SetMarkerSize(0.5);
  histTemp3a->SetMarkerColor(kRed);
  histTemp3a->SetLineColor(kRed);
  histTemp3a->SetName(Form("csv_MC_Pt%i_Eta%i",iPt,iEta));
...
//   histTemp4a->SetName(Form("csv_MC_nonzjets_Pt%i_Eta%i",iPt,iEta));

  TH1F *hClone = (TH1F*)histTemp3->Clone(Form("csv_ratio_Pt%i_Eta%i",iPt,iEta));
//   TH1F *hnonz = (TH1F*)histTemp4a->Clone();
//   hnonz->Scale(-1);
//   hClone->Add(hnonz);
...
//   }
  ...
  TCanvas * can1a = new TCanvas (canvName, canvName, 1000, 400);


  can1a->Divide(2,1);  
  can1a->cd(1);
  if (histTemp3->GetMaximum() < histTemp3a->GetMaximum())  histTemp3->SetMaximum(1.1*(histTemp3a->GetMaximum()));
  histTemp3->Draw("histE1");
  histTemp3a->Draw("histE1same");
  ...
  TLine * ll = new TLine(0,1,51,1);
  ll->SetLineColor(2);
  ll->Draw("same");
  TString plotName = Form("_Pt%i_Eta%i",iPt,iEta);
  can1a->Print("PU_test/sf_2jetsge2tags_"+varName +".png");
