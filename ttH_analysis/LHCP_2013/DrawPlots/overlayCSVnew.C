...
#include <algorithm>
  ...
  TString ofName = "csv_rwt_checkPt.root";
...
//   etaBins.push_back("1.6");
  ...
  TFile * sourceFile1a = new TFile ("/afs/crc.nd.edu/user/a/abrinke1/BEANsHelper/CMSSW_5_3_2_patch5/src/BEAN/DrawPlots/bin/limits_v1/TwoMuon.root");
  TFile * sourceFile1b = new TFile ("/afs/crc.nd.edu/user/a/abrinke1/BEANsHelper/CMSSW_5_3_2_patch5/src/BEAN/DrawPlots/bin/limits_v1/TwoEle.root");
  TFile * sourceFile1c = new TFile ("/afs/crc.nd.edu/user/a/abrinke1/BEANsHelper/CMSSW_5_3_2_patch5/src/BEAN/DrawPlots/bin/limits_v1/MuonEle.root");
  TFile * sourceFile2 = new TFile  ("/afs/crc.nd.edu/user/a/abrinke1/BEANsHelper/CMSSW_5_3_2_patch5/src/BEAN/DrawPlots/bin/limits_v1/allMC.root");
...
//   TFile * sourceFile2 = new TFile ("bin/dilSummaryTrees_"+MCsample1+"_"+year+"_"+label2+"_"+extraLabel+"all.root");
  ...
  int nBins = 20; //15; //Number of bins
...
//   TH1 * sourceHisto2a = (TH1*) summaryTree->Draw(varName2+" >> histTemp2a",nonttbar+lumi+"weight*(Xsec/nGen)*("+lepselection2+ " && "+trigselection+" && "+jetselection1b+")","");
  TH1 * sourceHisto3a = (TH1*) Tree2->Draw(varName1+" >> histTemp3a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && "+trigselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto4a = (TH1*) Tree2->Draw(varName2+" >> histTemp4a",nonttbar+lumi+"weight*topPtWgt*(Xsec/nGen)*("+lepselection2+ " && "+trigselection+" && "+jetselection2b+")","");
...
//   hClone->Write();
  ...
  plotName += "_iEta";
  plotName += iEta;
  can1a->Print("csvRWT_"+varName+"_"+WP+plotName +".png");
